 ...
 * running it once, and it is — by four separate mechanisms that were already
 * there:
 *
 ...
 *
 * This module adds no fifth mechanism of its own. It has no state.
 *
 ...
 *
 * Three of the four have a legitimate *"nothing to do"*: a week with no
 * publishable game cannot be finalized, a season with no open stakes settles
 * nothing, and a week whose facts refuse cannot be drafted. None of those is an
 * error and none of them stops the chain — they are recorded in the report and
 * the job carries on, because a Tuesday where the books are still open on one
 * game should still settle the bounty that cleared three weeks ago.
 *
...
export async function runTuesday(
  ...
  // --- 4. Draft the Slice, and stop at the desk ----------------------------
