Warning: truncated output (original token count: 63168)
...
async function reach(page: Page, state: StateName): Promise<void> {
  switch (state) {
    ...
    case 'door-claim': {
      await signOut(page);
      const unclaimed = page.locator('main li a').filter({ ha…43168 tokens truncated…n option in the open category that is **not** the current one.
 *
