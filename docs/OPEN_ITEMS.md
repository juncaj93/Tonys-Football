...
`docs/MANAGER_SPRITE_BOUNDARY.md` is the canonical account, including the ten
defects the render loop found and the four that no test in the suite could have.
**A tinted-mask pipeline is not refused — it is not needed**, and if painted
layers are ever commissioned this geometry is what they are painted to.

**Category A is now empty.** Every v1 system is built, tested and reachable;
what remains below is polish, activation, or deferred scope.
