...
- ~~**The opening balance is the only honest token source today.** Matchup wins and weekly high scores need a played season and the two cron jobs that would award them (`16 §4.3`) do not exist. The enum declares them; nothing is wired to them. **Do not invent a weekly reward that fires on nothing.**~~ **Resolved 2026-08-04.** Both crons exist (#56, #59), so the precondition this stated is met rather than waived, and `lib/rewards/` now pays matchup wins and weekly high scores from a finalized week. The rule itself still stands for everything it did not name — `SEASON_AWARD` remains declared and unwired for exactly this reason. See `docs/WEEKLY_REWARDS_BOUNDARY.md`.
