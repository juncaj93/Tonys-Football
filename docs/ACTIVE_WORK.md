...
|---|---|---|---|---|
| Slice restraint — drop the board, open Tony's Line on the paper, collapse the draft review | claude/tonys-pizza-slice-sim-3mzxm5 | `components/slice/*` · `lib/slice/render.ts` (presentation only) · `lib/flags.ts` (`tonysLine`) · `components/scene/stakes-band.tsx` · `docs/SLICE_RESTRAINT_BOUNDARY.md` (new) | `lib/stats/*` · story selection · the validator's rules · `Edition`'s shape or hash · the approval chain · schema · crons · art | 2026-08-11 |
...
| Tony's Line becomes personal · Tony's Chalkboard replaces the weekly-high call · playoff-period `EditionCharacter` (commissioner rulings, 2026-08-12) | claude/tonys-pizza-slice-sim-3mzxm5 | `lib/stakes/*` · `lib/stakes/propositions.ts` (new) · `drizzle/0019_personal_line_ownership.sql` (new) · `components/scene/chalkboard.tsx` · `components/slice/stakes-band.tsx` · `lib/slice/render.ts` + `components/slice/newspaper.tsx` (`postseason` character only) · `lib/simulation/*` · `lib/rehearsal/harness.ts` (the observed-stake record only, additively) · `docs/CHALKBOARD_BOUNDARY.md` (new) | the wager amount, the payout multiple, settlement timing, `apply_token_delta`, any idempotency guarantee · any second token market · `playoff_bracket` or any bracket persistence · story selection · significance thresholds · the approval chain · the Slice's art or typography · rooms · sprites · art · catalog · cron schedules | 2026-08-12 |
