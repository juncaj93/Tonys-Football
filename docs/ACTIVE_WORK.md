...
| Server-side weekly reward eligibility & early-dues bonus | claude/weekly-reward-eligibility-hkqk4s | `lib/rewards/` (weekly high-score eligibility, the early-dues thank-you grant, the first-login receipt read model) · one additive `token_reason` enum value · `scripts/seed.ts` wiring · `docs/WEEKLY_REWARDS_BOUNDARY.md` | box/rarity odds · casino odds or rounds · existing token amounts (150/400/200/salvage) · any route, page or visual component · character, rooms, navigation, dialogue, collectible systems | 2026-08-26 |
