...
| Commissioner per-manager correction tools | claude/commissioner-correction-tools-r2h5b1 | `app/admin/corrections/` · `app/actions/admin-corrections.ts` · `lib/admin/corrections.ts` (+ test) · the `reason` argument on `resetPin` in `lib/auth/service.ts` · the office's link to the corrections screen · three visual states in `scripts/visual-qa.mts` · `docs/COMMISSIONER_CORRECTIONS_BOUNDARY.md` | Token balances and the ledger, loot boxes, the casino, rooms, navigation outside `/admin`, art and the registry, player-facing dialogue and the content engine, the Slice review chain, the character customiser itself, and any bulk or league-wide reset | 2026-08-26 |
