...
| 2 | ~~Week 16's column reads *"An ordinary week, and Tony prints those the same as the other kind."* on a semifinal decided by 0.42.~~ **CLOSED by commissioner ruling, 2026-08-12 (Ruling 12).** `EditionCharacter` gained `postseason`, from `packet.weekType` — already in the packet, nothing new persisted, nothing inferred, and no round or elimination language anywhere. `docs/CHALKBOARD_BOUNDARY.md §3` | editorial — **resolved** |
...
| 4 | The stakes band can print **`TONY'S CHALKBOARD · WEEK 18`** beside **`THE BOUNTY · WEEK 17`**. That is a *rolling* bounty behaving exactly as `16 §38` specifies, and the two week labels side by side may read as staleness | editorial |
...
| 7 | Every personal line's explainer in the week-16 report reads **`6 of 6`**. That is `WEEK_16_SCRIPT`, not the formula: it generates scores from a fixed per-roster strength, so a manager's scores cluster tightly and a line pulled toward the league sits under all of them. The **same code on the real 2024 and 2025 seasons** gives the full 0-to-6 spread and a **48.5% over** rate | simulation input |
| 8 | **Week 1 is never authored.** The Tuesday job authors `week + 1`, so the earliest stake any season carries is week 2's — for the line and the bounty as well as the chalkboard. Pre-existing and unrelated to the 2026-08-12 rulings; visible now only because the chalkboard's history-free families made week 1 a question worth asking | operational |
