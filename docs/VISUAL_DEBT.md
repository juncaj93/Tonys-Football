...
| 7 | **The homepage, seconds after load — Tony clips briefly.** Observed hosted by the commissioner, 2026-08-01. **A separate defect from the hydration mismatch PR #50 repaired**, and not to be assumed fixed by it: `#418` is a tree disagreement at hydration, this appears seconds after mount. A single post-navigation screenshot cannot see it — the regression has to be timed or frame-sequenced, and it has to **fail on the old behaviour**. Investigation list, acceptance criteria and the required frames: `docs/HOMEPAGE_CLEANLINESS_BOUNDARY.md §1` | **Not minor** — it is on the first screen every manager sees, and it is listed here only because the direction that recorded it was explicitly non-interrupting. It leaves this list at the top of the next homepage slice. |
| 8 | **The homepage reads as burnt, scratchy and muddy where it should read as clean pixel art.** The Tonight board's face, the wall behind Tony, some surrounding room surfaces, and parts of Tony where texture or scaling smears him. The board should become a calm near-white or cream writing surface; the wall behind Tony should stop competing with his silhouette; Tony is **not** to be redesigned, only cleaned where rendering or texture dirties him. Needs no new art from the commissioner. Full direction and the mechanism-per-surface requirement: `docs/HOMEPAGE_CLEANLINESS_BOUNDARY.md §2`–`§6` | Recorded as non-interrupting. It is one coherent slice rather than four small fixes, and doing it piecemeal inside another milestone is how a room ends up half-cleaned. |
