...
| | What | Closed by |
|---|---|---|
| 19 | **`/back-hall?open=none`, all widths — tapping the shut stairs showed a line nobody could read, and at 390 it was not on the screen at all.** `ShutDoor` renders its in-world answer 8 room units below the door's rectangle; the stairs ended at `y 542`, so the panel's top landed at **670.3px in a 664px viewport** at 390, at 644.5px at 375 with all but 19px of its 68px height below the fold, and at 618.8px at 360 with the bottom 22px clipped. `18 §6.3` requires a locked door to answer *in world*, and an answer below the fold is the "coming-soon badge" failure reached from the opposite direction | **2026-08-11, by the geometry rather than by a patch.** Filed during the Back Hall art investigation and closed by the shell that came out of it: the painted stairwell is a framed opening in the wall rather than a hole in the floor, so both lockable doors now end at `y 292` and the stairs' line lands at **365.6–433.2** at 390. The rule survives the fix — a lockable door must end above `y 465`, asserted in `backhall.test.ts` and derived from the two numbers that produce it rather than written down. **No screenshot could ever have caught this**, which is the useful half: the line is at `opacity: 0` until something taps the door, and `back-hall-shut` has always been photographed without tapping anything. So `checkBackHall` **taps**, and restoring the old rectangle turns it red at all three widths with exactly the numbers above. `docs/BACK_HALL_BOUNDARY.md §10.3` |
