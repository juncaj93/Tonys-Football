...
| Date | Ruling | Where |
|---|---|---|
| 2026-08-12 | **COMMISSIONER: blackjack V1 is hit · stand · natural · bust · dealer draw · push**, dealer stands on **all 17**, natural pays **3:2**, wagers **20 / 40 / 80**, no double/split/surrender/insurance and **no placeholders for them**. A hand is durable server state with **at most one open per manager, enforced in the database** | `docs/CASINO_BOUNDARY.md §13` · `drizzle/0020_casino_blackjack.sql` |
| 2026-08-12 | **COMMISSIONER: an abandoned hand is stood, never refunded.** A refund is a free option — deal, see sixteen, walk away, recover the wager. Season close stands the player on their current total and runs the dealer normally. **No cleanup cron and no timeout job** | `settleOpenHandsAtSeasonClose` · `finalizeSeason` |
| 2026-08-12 | **`finalizeSeason` settles open hands *before* it shuts the books, in one transaction.** `apply_token_delta` refuses a finalized season, so a hand settled after the close could never be paid and `blackjack_hands_one_open_per_user` would jam that seat permanently | `lib/sleeper/persist.ts` |
| 2026-08-12 | **The action key is checked before the revision.** A committed action whose response was lost is retried with a revision stale by exactly one; checking the revision first turns every lost response into a spurious stale-tab refusal instead of the committed result | `lib/casino/blackjack.ts` |
| 2026-08-12 | **Season close must not depend on the casino being seeded.** Reading the blackjack rules unconditionally coupled every season close to a casino table nine existing tests had never stored. The rules are read only when there is an open hand | `settleOpenHandsAtSeasonClose` |
| 2026-08-12 | **The visual gate runs against a mismatched browser via `PLAYWRIGHT_CHROMIUM`**, which the driver already supported. No gate was weakened, rewritten or bypassed to reach green | `scripts/visual-qa.mts` |
| 2026-08-11 | **COMMISSIONER (second set): slots buttons 5 / 10 / 20, top prize 20× (400 at the top button), and 85.13% RTP is refused** — re-derive toward ≈92%, exploring 90 / 92 / 93, with no assumption that 92 wins. Blackjack is untouched at 20 / 40 / 80 and 2.03%. **Recommendation: candidate E at 92.09%** | `docs/CASINO_BOUNDARY.md §11` · `docs/evidence/casino/slots-redesign.md` |
| 2026-08-11 | **With a top prize fixed at 20×, a pair tier is arithmetically mandatory.** The whole triple tier can return at most `P(triple) × 20` = 70.0%, so no paytable reaches 90% on triples alone. Frequent small wins are determined by the rulings, not chosen for feel — asserted by test | `lib/casino/candidates.ts` · `lib/casino/casino.test.ts` |
| 2026-08-11 | **Leaving 85% did the work; the choice inside the 90–93 band did almost none.** Regular play's collection cost fell 40% → 12%, and the heavy-archetype gate failure at 85% disappeared. Across 90 → 93 a regular player gains 0.2 items for 611 tokens/season of lost sink | `docs/CASINO_BOUNDARY.md §11.4` |
| 2026-08-11 | **A single ten-manager season is too small to compare paytables.** Season-one item counts came out non-monotonic in the return rate — noise read as a finding. Now the mean over 50 independent leagues, 500 manager-seasons per figure. Same class as the 2026-08-10 gate correction | `scripts/slots-redesign.ts` |
| 2026-08-11 | **Correction: the Phase 2 report's "halves the biggest possible single win" was wrong.** It compared against an unapproved candidate. Against R5/R7 as written the maximum wager halves 40 → 20 while gross top payout doubles 200 → 400 and maximum net win rises 160 → 380. Wording corrected; analysis unchanged | `docs/CASINO_BOUNDARY.md §4` |
| 2026-08-11 | **COMMISSIONER: G1 is resolved — the Underground opens with blackjack and slots**, staked in fictional tokens. R1–R13 fix the ruleset (hit · stand · natural · bust · dealer draw · push · dealer stands on all 17; no split, double, surrender or insurance), the 3:2 natural, the absence of any progressive jackpot, net-negative expected value by design, no daily play cap, private history, both games before the curtain opens, and casino payouts excluded from the earned-token award. The 2026-08-09 reopening brief's *"avoid building slots, roulette, blackjack clone, poker"* is the **loser**, for the Underground only | `docs/CASINO_BOUNDARY.md` · `docs/OPEN_ITEMS.md` **G1** |
| 2026-08-11 | **A per-spin token ceiling and a maximum wager button jointly determine the top multiplier, and R5's 40 with R7's 200 leaves no top prize.** Five integer levels cannot carry six symbols plus a money-back tier, so the rarest combination ties with two commoner ones. A coherence audit refuses it and a test pins the refusal. **Recommendation: slots at 5 / 10 / 20 with a 400 ceiling, exact RTP 85.13%** — awaiting a second ruling | `lib/casino/slots-model.ts` · `docs/CASINO_BOUNDARY.md §4`, `§8` |
| 2026-08-11 | **The casino economy scenario is opt-in and draws from its own generator.** The approved gate must stay byte-identical with the casino off, and it does — 11.0 boxes, 52.9%, 6,976 openings, 2.76 legendaries. Proved twice: by restating the recorded figures, and by asserting that a league in which nobody plays is the *identical* league to no policy at all | `lib/economy/simulate.ts` · `lib/economy/casino-scenario.ts` |
| 2026-08-11 | **The token-leader award does not exist**, so R12 is discharged by an **allowlist** of earned reasons rather than a calculation change. An allowlist fails closed — a reason added later counts for nothing until somebody adds it deliberately; `all positive deltas except casino` fails open and fails silently | `docs/CASINO_BOUNDARY.md §7` |
...
| 2026-08-10 | **COMMISSIONER (R1): the empirical calibration wins; `07 §7.1`'s 20 / 40 / 60 bands are superseded and the question is closed.** Measured over the 162 real finalized games, the median margin is **~20.6** and **~54%** of games clear 20 — so a bare `>= 20` labels half the league a blowout, which is not a classification. The durable principle: *a significance threshold is deterministic, documented, and calibrated against the actual verified league distribution, and a label like "blowout" must identify meaningfully unusual outcomes rather than describe roughly half the games.* **No subjective AI classification of blowouts. No thresholds that move week to week.** A future recalibration is an intentional, documented product decision — never a silent change to what history meant | `lib/stats/significance.ts` · `docs/HISTORICAL_ANALYSIS_BOUNDARY.md §9` · PR #83 |
| 2026-08-10 | **COMMISSIONER (R2): the neutral play-everyone measurement stays, and the `07 §7.5` / `DATA_AUDIT §9` disagreement is resolved.** All-play is permitted as a **secondary contextual measurement**, explicitly labelled and derived from the same verified eligible games. It is **not** an alternative standings system, not a replacement for wins and losses, not a "true record", not a luck ranking, not a manager-quality ranking, not a fraud detector, and not evidence anybody "deserved" a different record. Verified Sleeper results remain authoritative. The deterministic layer supplies the measurement and the signed difference; the editorial layer may note the contrast but may not turn it into a factual claim about luck, skill, fraud or deservingness. **A week whose integrity makes the comparison unreliable is excluded whole**, never made asymmetric | `lib/stats/luck.ts` · `docs/HISTORICAL_ANALYSIS_BOUNDARY.md §9` · PR #83 |
| 2026-08-10 | **COMMISSIONER: the historical-analysis architecture is approved and is not to be redesigned.** Deterministic facts from the existing verified tables · explicit typed scope · permanent-manager resolution · disputed-data exclusion · mechanically banned overclaims · no invented relationships · **no `league_events` spine** · no AI prose as a source of fantasy fact · an unsupported-story registry. The scope validator may **not** be weakened for editorial convenience, and infrastructure may not be created merely to make an unsupported story possible | `docs/HISTORICAL_ANALYSIS_BOUNDARY.md` |
...
| 2026-08-01 | **The sign carries the prediction and the market; the paper carries the bounty.** `18 §3.4` fixes the sign's two things and `16 §38` puts all three on the Slice. A third item on a 37-unit sign would absorb another surface's scope into a homepage `18 §3` fixes at eight objects | `components/scene/chalkboard.tsx` · `components/slice/stakes-band.tsx` |
| 2026-08-01 | **A stake resolves once, by natural key.** `stake_resolutions.stake_id UNIQUE`, inserted before any token moves — `0004`'s argument for `box_openings.box_id`, not `0005`'s caller-supplied key. Ten parallel settlements produce one payout, asserted | `drizzle/0010` · `lib/stakes/settlement.test.ts` |
| 2026-08-01 | **The line is a lower median, never an average.** An averaged median of an even count lands between two real scores. The lower median **is** one of the season's scores — exact in cents, and a number somebody actually posted. "Half above, half below" becomes "at least half above", which is inside `16 §9`'s *structurally ~50/50* and is stated rather than implied | `lib/stakes/facts.ts` |
| 2026-08-01 | **A stored config missing a key is as loud as a missing config.** `economyFor` cast jsonb to `EconomyValues` with no evidence; the first value ever *added* to the economy left every earlier environment serving a row without it, and the market printed *"undefined off your tab either way"* on its own price line. Append-only versioning keeps old numbers interpretable and says nothing about a row being complete for today's code | `lib/counter/tokens.ts` |
| 2026-08-01 | **A rolling bounty is answered, not delayed.** A week that finished and did not produce a claimant is `rolling`, not `awaiting-final`. Reporting *"still being counted"* about a closed week is false in the quietest possible way, and it is the sentence a manager would read most often | `lib/stakes/model.ts` |
| 2026-08-01 | **Curated stake copy lives in one module, and the validator sees all of it.** `resolve.ts` chooses an evidence key and supplies values; it writes no sentence. A resolver holding its own strings would have its explanation validated against templates it supplied — a layer marking its own homework | `lib/stakes/copy.ts` |
| 2026-08-01 | **COMMISSIONER: homepage visual cleanliness.** The homepage reads as burnt, scratchy and muddy where it should read as clean pixel art — the Tonight board's face, the wall behind Tony, and parts of Tony himself. The board becomes a calm near-white or cream writing surface; **Tony is not redesigned**, only cleaned where rendering or texture dirties him. Readability and intentional pixel structure beat preserving grunge. **No new art from the commissioner**, and the mechanism chosen per surface is part of the deliverable — *"do not cover a source-art defect with a fragile CSS patch"* | `docs/HOMEPAGE_CLEANLINESS_BOUNDARY.md` · visual debt 8 |
| 2026-08-01 | **COMMISSIONER: Tony clips briefly seconds after the homepage settles**, observed hosted. **A separate defect from PR #50's hydration repair** and not to be assumed fixed by it — `#418` is a tree disagreement at hydration, this appears long after. It is transient, so **one post-navigation screenshot is not evidence**: the regression must be timed or frame-sequenced across six moments at three widths, and it must **fail on the old behaviour** | `docs/HOMEPAGE_CLEANLINESS_BOUNDARY.md §1` · visual debt 7 |
| 2026-08-01 | **Direction can be recorded without being acted on.** This one arrived explicitly non-interrupting, and the correct response was to persist it in the checkpoint and the visual-debt system and let the in-flight gate finish. A ruling that stops a running gate to be obeyed immediately costs the gate's evidence and buys nothing | `AUTONOMY.md §0` — the repository is the state machine |
