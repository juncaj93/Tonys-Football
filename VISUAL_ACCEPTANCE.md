...
Plus the **draft-review special** (`docs/PRESEASON_SLICE_BOUNDARY.md`), nine states:

- **the issue** — `slice-preseason-draft-review` and `slice-preseason-sparse`, both via `?edition=`, writing nothing. The second is the one that matters: the best pick and the concern are *optional*, so the page has to look finished when eight of ten sections carry neither, and that is the state Tony really files.
