...
Plus the **press desk** — the commissioner review chain (`16 §9`, `08 §22`), seven states: `review-empty` · `review-waiting` · `review-held` on the desk itself, and `review-draft` · `review-refused` · `review-approved` · `review-published` on the draft's own screen.

Four of them are photographed on the **draft** rather than on the queue, and that is a finding rather than a preference: an issue belongs to the **league**, not to a seat, so the desk's sections accumulate as each state is applied and by the last one every desk looks the same. Four states photographed byte-identically before this was fixed. The screen where *"approved"* means something is the one with the Print button on it.

`review-empty` is a **preview parameter** (`?desk=empty`) for the same reason: the driver loops widths on the outside, so an empty desk cannot survive its own run. It writes nothing.

Every press-desk seat carries the commissioner's keys. `requireAdmin()` answers `notFound()`, so a seat without them renders a **404** that photographs cleanly, files under the state's name, and passes every pixel gate — which is why `review-desk` checks markers rather than pixels.

Plus the **draft-review special** (`docs/PRESEASON_SLICE_BOUNDARY.md`), ten states:

- **the issue** — `slice-preseason-draft-review` and `slice-preseason-sparse`, both via `?edition=`, writing nothing. The second is the one that matters: the best pick and the concern are *optional*, so the page has to look finished when eight of ten sections carry neither, and that is the state Tony really files. It also carries the **longest names this league has** (borrowed from the weekly board's own list, so the phrase means one thing in this product) and **every take at exactly the 240-character limit** — the two stresses together rather than apart, because a long name is hardest in a section with nothing else in it to push the layout around.
- **the body of it** — `preseason-teams`, the same paper scrolled to its **tenth** section. Captures are viewport-sized, so the two states above photograph the masthead and nothing below it — and the draft review's entire body is ten sections under that fold. The tenth, because it is the one a short list would be missing and the one a scroll has to survive to reach. Same defect and same fix as `review-decision`: a state that scrolls, not a taller picture.
- **the board** — `draft-board-empty` · `draft-board-partial` · `draft-board-complete`, which are `0`, `4` and `10` of ten. Three screenshots that differ by two glyphs at thumbnail size, so the counts are read from the DOM and asserted **exactly**. The print control's *absence* at the first two is what makes its presence at the third mean something.
- **the editor** — `draft-editor-blank` and `draft-editor-written`, two rows of the same board, opened from it. Thirteen grades in the grid and the draft on the same screen as the take are both checked in the DOM: at 360 the fourth column of the grid is the one at risk and a missing one is invisible.
- **the ends of the chain** — `preseason-review` on the desk, `preseason-rack` on the shelf. The first proves the special edition needs the same stamp; the second proves it reaches a reader only after somebody presses it.
...
| `one-transient` | At most **one transient surface is visible** at a time — `MANDATE §6`. Opacity is followed through every ancestor and zero-area elements are ignored, because the pad **yields rather than unmounting**: it stays in the tree at `opacity: 0` so its state survives and it comes straight back | Every state at every width, not only the one built to catch it — the version of this defect that shipped was invisible precisely because only the state designed around it was ever checked. The new `display-over-tony` state opens a Display **without** dismissing Tony first, which every other Display state does and which is why visual debt 4 survived every screenshot. It fails on the pre-pass build at all three widths: *"shows 2 transient surfaces at once (panel + tony-line)"*. See `docs/ROOM_TRANSIENTS_BOUNDARY.md §8` |
| `focus-visible` | The element focused after tabbing into the room is a **room object** and carries an outline of at least 2px that is not transparent | WCAG 2.4.7 and `MANDATE §6`'s *"focus states stay usable, appear only during relevant interaction, and stay pixel-aligned"*. `.room-shape:focus-visible .door-edge` was the ring; `18 §9.4` withdrew the polygons it was painted on, nothing else in the stylesheet styled focus, and every room object carries `outline-none` — so tabbing through the parlor moved an invisible cursor. The `keyboard-focus` state photographed exactly that and passed, because a screenshot only fails when somebody looks at it. Fails on the pre-pass build at all three widths |
...
| `tony-glow` | Across the glow's fade, **no pixel inside Tony's own bounds is darker than the settled room**, on the `tony-steady` state at all three widths | `drop-shadow` composites a blurred copy of the alpha *behind* the element, so it can only ever add light — a pixel that goes **darker** cannot be the glow, and needs no authored mask to identify. That is the whole discriminator. It measures compositor frames over CDP rather than `getBoundingClientRect`, because the defect it catches moves **no rectangle**: a `filter` transition promotes a compositing layer, the layer rasterizes a fractional-size sprite on a different pixel grid, and his edges step one device pixel against the counter that cuts him. Fails on the pre-fix build with 27 / 320 / 90 darker pixels. `docs/HOMEPAGE_CLEANLINESS_BOUNDARY.md §11`. |
...
| `room` | Every `room*` state carries the whole eight-object map, is on the route its name claims, shows **exactly the number of things the state says are on show**, is drawn in the theme it names, and — on a visited room — offers **no control that would change it** | The basement's states are built by *pressing the room's own controls*, so the gate has to count rather than trust the drive. It caught the first defect on its first run: the driver's filler matched `Move X here` as well as `Put X here`, so after the first place was filled it took the row holding the item already in it and each place stole the last one's contents — one thing in the room, four times over, on a page that looked entirely plausible. The theme is read from `data-room-theme` for the reason `slice-edition` is read from the DOM: three themes that all rendered the storeroom would be three identical screenshots under three names. `lib/rooms/driver-coverage.test.ts` closes the other half — a state photographed but never checked, or a theme with no state at all. |
...
| `reduced-motion` | With `prefers-reduced-motion: reduce`, **nothing is still animating** once the room has settled — and, on the same page with motion allowed, **something is**. Both halves, at all three widths | An accessibility promise the product makes in **seven places** — four `@media (prefers-reduced-motion: reduce)` blocks in `globals.css` plus JavaScript branches in `arrival.tsx`, `spoken-line.tsx` and `counter-tray.tsx` — across **seventeen** keyframe animations, and until this gate nothing verified a single one of them. It reads `document.getAnimations()`, which is what the compositor is actually running, rather than auditing the stylesheet: the likeliest future regression is `element.animate()`, and the Web Animations API is exactly what `animation-duration: 0.01ms !important` **cannot** touch. The control is the load-bearing half — "nothing is animating" passes trivially if the room stops animating at all, so the same page is sampled twice and the motion-allowed pass must find the two haze drifts and the sign's sway alive. Proven by injecting a `body.animate()` loop into `room-stage.tsx`: it fails at all three widths with *"1 animation(s) still running under prefers-reduced-motion: script-driven"*, and passes once removed. |
...
- **Text uncomfortable to read on an iPhone.** Body and dialogue copy sit at **16–18 CSS px, adjusted upward when the pixel font needs the optical size** (`PRODUCT_DELIVERY_MANDATE.md §6`, which supersedes the earlier bare "17px floor"). Size the container to the type, never the type to the container — that mistake was made once at 15px to shrink a box, and again at 8px and 9px on the Tonight board.
  ...
  **This is now mostly structural rather than a judgement.** Type is set from a **role** in `lib/design/type.ts` — `TYPE.body`, `TYPE.sectionHeading`, `TYPE.ledgerValue` — and nothing else in the product names a font size. Six sizes (`13 · 15 · 17 · 19 · 22 · 26`), down from sixteen. `lib/design/typography.test.ts` fails the build on a size at a call site and the `type-floor` gate fails it on a rendered one. What is left for a reviewer is what a machine still cannot judge: whether the **right role** was chosen. A sentence set in `eyebrow` is signage somebody mistook for prose, and no gate will say so. See `docs/TEXT_SURFACE_BOUNDARY.md`.
...
**Typography** — the **role**, first: is this a heading, a ledger key, a badge, a sentence? Then readability · hierarchy · contrast · wrapping · truncation · alignment. Look specifically for light-on-light, dark-on-dark, patterned grounds interfering with text, dither under small type, labels disappearing into artwork, and a **paragraph set in the display face** — Silkscreen is half again as wide per character as VT323 and runs out of a 360px phone in about twenty-six characters, so a sentence in it is a sign somebody mistook for prose.
