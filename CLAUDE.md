...
**Next assignment:** **#26 — the Stats Intelligence deterministic fact layer**, sequenced ahead of everything narrative because `MANDATE §9`–`§10` forbid the interface deriving a fantasy fact for itself. `boardFace()` already renders nothing where the matchup of the week belongs; filling that socket is the layer's acceptance test. Then **#24 — M3 modular character identity**. See `docs/CHECKPOINT.md` for the durable state.
