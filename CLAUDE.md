...
- **The manager sprite is a genuine architecture decision, not a swap.** Colour is a runtime parameter — 4 skin × 8 hair × 8 top ramps resolved at render — and a PNG layer bypasses it entirely, so the existing per-layer art-swap would silently delete seven of eight hair colours. Keeping the traits *and* using PNGs is **132 files** before a single wearable. `docs/ROOMS_BOUNDARY.md §14` sets out the two real routes and what each costs. **Nothing in the customiser was touched** — it stays `CLOSED`.
